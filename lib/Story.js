  ...
  constructor (dataEl) {
    ...
    $(window).on('sm.story.error', (event, error, source) => {
      ...
      if (this.ignoreErrors === false) {
        console.warn('In '+source+': ', error);
        var errorMessage = this.errorMessage;
        _.defer(() => { // defer to set it after render()
          var $t = $('tw-passage');
          if (0 == $t.length) $t = $('tw-story'); // if tw-passage does not exist yet, use tw-story
          $t.first().append($('<div class=sm-error>').append($('<code>').text(errorMessage)), '\n');
        });
      }
    });
  }
