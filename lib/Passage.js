...
class Passage {
  ...
  render (source) {
    ...
    var passageSource = !(typeof source !== 'undefined' && source !== null);
    if (passageSource) {
    ...
    try {
      ...
      var passageHint = passageSource ? '(of "'+this.name+'") ' : '';
      $.event.trigger('sm.story.error', [error, 'Passage.render() '+passageHint+'using _.template()']);
    }
